...
- Homebrew cask and formula support, ported from the `3.6.0_Homebrew` branch onto the 3.8.0 line and reworked for the machinery added since:
	...
	- Homebrew runs as the owner of the prefix (`stat -f %Su`), not as the console user the source branch assumed. A root-owned prefix disables Homebrew entirely, because `brew` refuses to run as root. A prefix owned by someone other than the console user still has its formulae managed as that owner - only casks are held back for that account (`brewCasksAllowed=FALSE`), and the reason is logged
	- The owner-vs-console-user split exists because `sudo -u <owner> -H brew upgrade <formula>` needs no GUI session at all - it is a plain command-line operation regardless of who is logged in. `brew upgrade --cask` is different: it uninstalls then reinstalls, and a cask's own `uninstall quit:`/`signal:`/`launchctl:` stanzas run as the brew user against that user's own Aqua session and launchd domain, so when the brew user has no GUI session those stanzas cannot reach a copy of the app running in the console user's session, and Homebrew replaces the bundle underneath it anyway. That loss of Homebrew's own teardown safety net - not TCC or `/Applications` permissions, which depend only on group membership - is why casks are held back for a foreign-owned prefix. `brew_cask_app_is_running` only guards the silent background patching path; the interactive dialog-driven install has no running-app check of its own and relies entirely on Homebrew's quit stanza, which is exactly what stops working across sessions. An earlier version of this gate skipped Homebrew outright whenever the prefix owner was not the console user, which disabled formula management even though formulae carry none of the risk the gate was meant to guard against
	- `homebrew_discovery` carried a second, broader guard inherited unchanged from the source branch: `[[ -z "${currentUserAccountName}" ]] || [[ "${currentUserAccountName}" == "FALSE" ]]` returned immediately, skipping Homebrew - formulae included - whenever `get_logged_in_user` had set `currentUserAccountName` to its own sentinel `FALSE` (the login window, a session that just ended, or a system console user such as `root`/`loginwindow`/`_mbsetupuser`). Since AAP's LaunchDaemon runs `aap-starter` every 60 seconds as root in the `system` domain and fires whenever `NextAutoLaunch` is due - routinely outside a user session - this silently patched zero formulae on most runs in an unattended deployment. It also had the safety backwards: with nobody logged in, no application is running, so it is the safest moment to patch, not the least safe. The guard is removed; the headless case now falls through unchanged into `get_homebrew_binary`'s existing `owner != currentUserAccountName` branch (the prefix owner is never literally the string `FALSE`), landing on exactly the same `brewCasksAllowed=FALSE` outcome as a foreign console user - formulae proceed, casks are held back. Casks being withheld here is a deliberate simplification, not a technical requirement: with nobody logged in nothing is running, so a cask's own quit/signal/launchctl teardown has nothing to interrupt, but "formulae only whenever the prefix owner is not the console user" is one rule to reason about rather than one that varies by who is at the screen, and some casks do user-level postinstall work that assumes a session exists. The one behavioural change is the warning text: it now branches on whether `currentUserAccountName` is `FALSE` ("no user is currently logged in") versus a real, different account ("the console user is '<name>'"), so the log no longer reads as though a Mac has an account literally named `FALSE`
...
- Fixed: two related defects made Homebrew discovery completely inoperative on every real Mac, found by a live run (72 outdated formulae, 5 outdated casks, discovery logged "queued 0 cask(s), 0 formula(e)"), invisible to the test suite because every JSON fixture was hand-written and omitted fields real brew always sends:
	- `homebrew_parse_outdated_json` piped `brew outdated --json=v2` straight into `plutil -convert xml1`. Real brew output puts `"pinned_version": null` on every single entry (pinned/pinned_version are only non-null for a pinned package), and property lists have no null type, so plutil rejected the entire document the instant one null was present - not just that field, every formula and every cask in the run. The fix sanitizes `: null` (a JSON null can only ever appear as a value immediately after a colon, so this cannot false-positive inside a quoted string) into `: ""` with `sed` before handing the document to plutil. The three fields this function reads - name, installed_versions[0], current_version - are never null in brew's output, so blanking null wherever it appears cannot corrupt a value the function depends on
	- `homebrew_discovery` consumed the parser through `done <<< "$(homebrew_parse_outdated_json ...)"`, which discards the command substitution's exit status entirely. A total parse failure and brew genuinely reporting nothing outdated therefore produced the identical log line, so the plutil defect above ran silently on every affected Mac with no warning and no error - the log read like a healthy no-op. The fix captures each parse into a variable first, checks its exit status explicitly, and on failure logs an ERROR naming which package type failed to parse and aborts discovery, instead of falling through to log "complete" as if the queue were legitimately empty
	- Both fixtures in `test_parse_outdated.zsh` and `test_discovery.zsh` are now modelled on a real `brew outdated --json=v2` capture, including `"pinned": false` and `"pinned_version": null` on every entry, and new tests assert the parser succeeds on a null-bearing payload and that a parse failure surfaces as an error rather than being swallowed
- Fixed: a comma in the SF Symbol icon specs Homebrew pseudo-labels emitted (`SF=terminal,colour1=#f5a623`, `SF=shippingbox.fill,colour1=#f5a623`) corrupted the swiftDialog `--listitem` line, which is itself comma-separated - `colour1=#f5a623` was read back as a second, bogus list item rather than part of the icon field, so a queued formula or cask could show a stray `colour1` entry in the pending-updates dialog. Found on a live run. `resolve_brew_icon_path` and `resolve_app_icon_path`'s Homebrew fallback now emit the bare SF Symbol name with no colour suffix; formulae and casks remain visually distinguishable by symbol alone. New tests assert no icon spec either function returns for a brew label contains a comma
- Fixed: the "Preparing updates" staging/silent-patch progress window opened in `main()` (InteractiveMode 2, with queued labels) unconditionally, before either of the two workflows it is a progress indicator for - `workflow_stage_updates` (`WorkflowStageUpdatesOption`) and `workflow_silent_patch_closed_apps` (`WorkflowBackgroundPatchClosedAppsOption`) - were checked. When both are disabled, neither block runs, and the window is opened and closed back-to-back with no work in between. `swiftDialogStagingWindow` launches swiftDialog in the background (`&`); the close path (`swiftDialogCompleteDialogStaging`) writes `quit:` to the dialog command file and deletes the file roughly 200ms later. With nothing running between open and close, that happens before swiftDialog has finished starting and begun reading the command file, so it never sees the `quit:` and the window is left on screen, overlapping every dialog shown for the rest of the run - reproduced live with `WorkflowStageUpdates=false` and `WorkflowBackgroundPatchClosedApps=false`. With either workflow enabled, the work it does between open and close gave swiftDialog enough time to start, which is why this was never noticed before. Fix gates the window open on `WorkflowStageUpdatesOption == "TRUE" || WorkflowBackgroundPatchClosedAppsOption == "TRUE"`, alongside the existing `InteractiveModeOption == 2` and queued-label checks; `stagingWindowOpened` tracking and the close block are unchanged. Provenance: pre-existing in 3.8.0, confirmed by `git blame` on both the open and close blocks; unrelated to Homebrew support, found during live testing of the Homebrew branch the same way the `_resolve_label_staging_info` `mktemp` defect above was. The underlying race - a window whose contents finish in well under ~200ms can still lose the same race even with a workflow enabled - is out of scope here; it is shared dialog-timing code used by all three progress windows (discovery, staging, patching) and is left for a separate fix. This code lives in `main()`, outside the block the automated test suite exercises, so it is not covered by an automated test; the guard's boolean logic was verified in isolation against all four combinations of the two options
...
- Ported [#254](https://github.com/App-Auto-Patch/App-Auto-Patch/issues/254) from 3.7.0 onto the 3.8.0 line (retains ScheduleWorkflowActive / SilentOutside from build `3.8.0.2608081340`):
	...
	- Treat `IgnoredLabels="*"` as an in-memory mode flag (single plist entry) instead of expanding into ~1,200 `PlistBuddy` writes
	- Protect `RequiredLabels` from wildcard ignore matching; harden local preference reads / `AAPPatchingStartDate` validation; strip `defaults read` array punctuation from label lists
	- See the 3.7.0 build `3.7.0.2608081505` entry below for the full root-cause write-up

### 08-Aug-2026 (1) - Build 3.8.0.2608081340
- [#166](https://github.com/App-Auto-Patch/App-Auto-Patch/issues/166): `ScheduleWorkflowActive` (SUPER-compatible workflow schedule windows) on the 3.8.0 line:
