...
- Homebrew cask and formula support, ported from the `3.6.0_Homebrew` branch onto the 3.8.0 line and reworked for the machinery added since:
	...
	- The owner-vs-console-user split exists because `sudo -u <owner> -H brew upgrade <formula>` needs no GUI session at all - it is a plain command-line operation regardless of who is logged in. Only a cask upgrade can touch a GUI session, since it replaces an application bundle that may be showing on someone's screen; that per-cask "is it currently running" risk is independently covered by `brew_cask_app_is_running`, whose `pgrep` check is system-wide and sees another user's session too. An earlier version of this gate skipped Homebrew outright whenever the prefix owner was not the console user, which disabled formula management even though formulae carry none of the GUI risk the gate was meant to guard against
